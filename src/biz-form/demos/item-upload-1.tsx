...
import { blobToDataURL, waitTime } from 'util-helpers';
...
const Demo = () => {
  ...
  const onPreview = React.useCallback(async (file: UploadFile) => {
    let preview = file.preview || file.url;
    if (!preview) {
      preview = await blobToDataURL((file.originFileObj || file) as File);
    }
